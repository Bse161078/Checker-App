...
let BathroomService = class BathroomService {
    ...
    async create(createBathroomDto, files) {
        ...
        const bathroom = await this.findOneByCheckerAndHotel();
        ...
        if (bathroom) {
            const updatedResult = await this.bathroomRepository.updateOne({ _id: bathroom._id }, { $set: newDto });
        }
        else {
            const createdResult = await this.bathroomRepository.create(newDto);
        }
        return true;
    }
    async getBathRoomStatus() {
    ...
    async findOneByCheckerAndHotel() {
        const { hotel, _id: checker } = this.request.user;
        const bathroom = await this.bathroomRepository.findOne({ hotel, checker });
        return bathroom;
    }
};
...
//# sourceMappingURL=bethroom.service.js.map