...
export class ReceptionService {
    constructor(
        ...
        @InjectModel(User.name) private cleanerRepository: Model<UserDocument>,
    ...
    async deleteReception(receptionId:string) {
        const user = this.request.user;
        const hotel = user.hotel;
        const reception = await this.cleanerRepository.findOneAndRemove({_id:new Types.ObjectId(receptionId)});
        return reception;
    }
